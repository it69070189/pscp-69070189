...
```text
โจทย์บอกให้คำนวณเงินก้อนสุดท้ายที่เราจะต้องจ่ายหลังจากคิด service และ vat

Input:
รับจำนวนเงินของลูกค้า

Output:
แสดงค่าที่คิด service และ vat ออกมา
...
```text
step1:รับ int
step2:กำหนด service_charge และ vat
step3:เช็คว่า service_charge < 50 หรือไม่
step4:ถ้าจริง จะกำหนดค่า service_charge ให้เป็น 50 เพราะขั้นต่ำคือ 50
step5:ถ้าไม่ตรงเงื่อนไข ให้เช็คว่า service_charge > 1000 หรือไม่
step6:ถ้าจริง จะกำหนดค่า service_charge ให้เป็น 1000 เพราะโจทย์กำหนดให้เก็บค่า service ได้ไม่เกิน 1000
step7:เอาผลที่ถูกคำนวณจาก service_charge ไปคำนวณ กับ vat
```
...
```text
วิธีสุดท้ายเหมือนกันเลยกับแผนแรก คำนวณ service โดย คำนวณ percent ของราคา และนำ service มาเช็คว่าได้ขั้นต่ำหรือได้ค่าสูงสุดที่โจทย์กำหนดหรือไม่ จากนั้นเอาผลนั้นมาคำนวณกับ vat เพื่อให้ได้คำตอบสุดท้าย
```
...
```text
เช็คค่า service_charge ขั้นต่ำ คือ 50 จากราคาที่ต้องจ่าย
```
...
```text
เช็คค่า service_charge สูงสุดได้แค่ 1000 จากราคาที่ต้องจ่าย
```
...
```text
เอาราคาสินค้า บวกกับ service_charge ที่ต้องจ่าย แล้วนำไปคูณกับ vat ของราคาที่ถูกคำนวณกับ service_charge แล้ว
```
