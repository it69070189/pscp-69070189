...
```text
input:
รับขนาดพื้นที่ของหมึก และ จำนวนพิกัดว่ามีกี่พิกัดว่าจะต้องคำนวณ
output:
แสดงเวลาที่พิกัดนั้นๆ จะถูกทับในเวลาที่เท่าไหร่
constraints:
ค่าพื้นที่ และ จำนวนพิกัด > 0 และต้องเป็นจำนวนเต็มเท่านั้น 
```
...
```text
step 1: รับขนาดพื้นที่ของหมึก และ จำนวนพิกัด
step 2:  กำหนด ค่า Pi ตามที่โจทย์กำหนด
step 3: คำนวณจากจุด (0,0) ไปยังจุด (x2,y2) จะใช้น้ำหมึกเท่าไหร่ถึงจะถึงจุดนี้
step 4: คำนวณเวลาที่จะถึงพิกัด (x2,y2)
step 5: ปัดเศษขึ้นหลังจากได้เวลาที่จถึงพิกัด (x2,y2)
```
...
```text
เหมือนกับแผนแรก รับค่าพื้นที่ และ จำนวนพิกัดที่จะต้องคำนวณ แล้วหาระยะทาง จากจุด (x1,y1) จนถึง (x2,y2) แล้วหาปริมาณของน้ำหมึกที่จะถึงพิกัด และคำนวณเวลาที่จะถึงพิกัด แล้วปัดเศษขึ้น
```
...
```text
Input ในบรรทัดแรกที่รับจะต้องเป็นจำนวนเต็มบวก และ > 0
```
...
```text
เวลาที่คำนวณได้จะต้องปัดเศษขึ้น
```
...
```text
หาก ค่าพื้นที่ (N) หรือจำนวนพิกัด (S) ที่ต้องคำนวณ มีค่า <= 0
```
...
```text
ช่วยอธิบายขั้นตอนต่างๆ ที่มาของสูตร
```

คุณยังทำอะไรด้วยตนเอง

```text
ทำ submission ด้วยตนเอง และเขียนขั้นตอนการคำนวนด้วยตนเองและได้ความเข้าใจกับคำอธิบายกับคำแนะนำของเพื่อน
```
