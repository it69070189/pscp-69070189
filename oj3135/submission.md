...
```text
input:
รับค่า n k t เป็น จำนวนคน ค่าที่ส่งต่อให้คนถัดไป เป็นโจร ตามลำดับ
output:
แสดงค่าว่า ค่าที่จะแสดงผ่าน n k t กี่ครั้ง
constraints:
n k t ไม่สามารถติดลบได้
```
...
```text
Step 1: รับค่า n k t
Step 2: ตรวจสอบว่า = 1 มั้ย หรือ = n มั้ย หากเท่ากับ ให้หยุดการทำงาน
Step 3: บวกครั้งที่นับในการลูปแต่ละครั้งทีละ 1
Step 4: แสดงผลค่าลูปที่ผ่าน n, t ครั้งจนกว่าจะครบเงื่อนไข
```
...
```text
คล้ายกับแผนแรกแต่ เพิ่มการตรวจสอบว่า + k ไปจะเป็นคนที่เท่าไหร่แล้วลูปไปเรื่อยๆ นับครั้งละ 1 จนกว่าจะกลายเป็น false ตามเงื่อนไขดังนี้ [+ k ไป แล้วค่ากลับมา = 1 หรือ + k ไปค่า = t ]
```
...
```text
ค่าไม่สามารถติดลบได้ แต่หากกรอกค่าติดลบไปยังคงแสดงผลที่นับลูป n ครั้งอยู่ดี
```
...
```text
หากค่า input = 0 จะเกิด ZeroDivisionError
```
...
```text
หาก input ที่กรอกไป ไม่ครบ 3 argument จะเกิด ValueError
```
