...
```text
Input:
บรรทัดแรก จำนวนเต็มสองตัวคือ จำนวนร้าน และ จำนวนเวลาที่ต้องเช็ค
ต่อมา num บรรทัด: เวลาเปิด กับเวลาปิด ของแต่ละร้าน
บรรทัดสุดท้าย เวลาที่ต้องการเช็ค check ค่า

Output:
พิมพ์จำนวนร้านที่เปิดอยู่ ณ แต่ละเวลาที่เช็ค เรียงตามลำดับ คั่นด้วยเว้นวรรค

Constraints:
ร้านถือว่าเปิดอยู่เมื่อ start <= time < stop คือรวมเวลาเปิดแต่ไม่รวมเวลาปิด
```
...
```text
Step 1: อ่าน num กับ check
Step 2: วนอ่านช่วงเวลาเปิด-ปิดของแต่ละร้าน เก็บเป็น list of tuple
Step 3: อ่านเวลาที่ต้องเช็คมาเป็น list
Step 4: วนแต่ละเวลาที่เช็ค แล้ว nested loop ไล่เช็คทุกร้านว่า start <= time < stop หรือไม่ นับจำนวนร้านที่เข้าเงื่อนไข
Step 5: พิมพ์ผลลัพธ์ทั้งหมดในบรรทัดเดียว คั่นด้วยเว้นวรรค
```
...
```text
ใช้เหมือนแผนแรกเป๊ะเลย คือ loop ซ้อน loop ไล่เช็คทุกร้านสำหรับทุกเวลาที่ query ไม่ได้ปรับอะไรเพิ่ม เพราะคิดว่า brute force ก็น่าจะพอสำหรับ time limit 1 วินาที มีจุดนึงที่สังเกตเห็นคือบรรทัด check += 0 ซึ่งไม่ได้ทำอะไรเลย ไม่มีผลกับผลลัพธ์ น่าจะเป็นโค้ดที่ค้างมาจากตอนลองดีบักแล้วลืมลบทิ้ง
```
...
```text
เช็คว่านับจำนวนร้านที่เปิดซ้อนกันถูก
```
...
```text
เช็คว่า stop ไม่รวม ถ้าเวลาเช็คตรงกับ stop พอดีต้องไม่นับร้านนั้น
```
...
```text
เช็คว่า start รวมด้วย ถ้าเวลาเช็คตรงกับ start พอดีต้องนับร้านนั้น
```
