...
```text
Input :
รับแม่สี เพื่อที่จะนำไปผสม
Output :
นำแม่สีมาผสมและแสดงผลที่นำมาผสมกัน หากตรวจแล้วไม่ใช่แม่สีจะ Error และหากสีที่นำมาผสมซ้ำกันจะได้สีเดิม
Constraints :
input ที่กรอกต้องเป็นแม่สีเท่านั้น และ  ควรพิจารณาหากสีที่นำมาผสมว่าซ้ำกันมั้ยและใช่แม่สีรึเปล่า
```
...
```text
step 1:รับแม่สี และรับมาโดยใช้ method capatilize()
step 2:ทำการสร้างเงื่อนไข และนำตัวแปรแม่สีมาเช็ค
step 3:ถ้าสีที่รับเข้ามาเข้าเงื่อนไขใดๆ จะผสมสีที่ได้ตามนั้น โดยต้องเป็นแม่สีเท่านั้น
step 4:ถ้า input ที่รับเข้ามาไม่ใช่แม่สี และ ไม่ตรงกับเงื่อนไขใดๆเลยจะแสดงผล Error

ก่อนถามเพื่อน ฉันทำเสร็จเกือบหมดแล้วแต่ฉันยังติด testcase ที่ สีซ้ำกันจะได้อะไรอยู่แค่ 3  testcase และเพื่อนแนะนำให้ใช้วิธีในการสร้าง set แทนเพราะ set มีคุณสมบัติที่ element จะไม่สามารถซ้ำกันได้
...
```text
ทำการสร้างตัวแปรเก็บ set ของแม่สี และนำมาตรวจสอบว่าเข้าเงื่อนไขหรือไม่
```
...
```text
ตรวจว่ารับสีที่ซ้ำกันมาพร้อมกันจะได้อะไร 
```
...
```text
ค่าที่รับเข้ามาเป็นแม่สี และอีกสีที่รับมาไม่ใช่แม่สี
```
...
```text
ข้อความสีที่รับมาตรงกับเงื่อนไขหรือไม่ ถึงแม้จะเป็นแม่สีก็ตาม เช่น พิมเล็กทั้งหมด ใหญ่ทั้งหมด หรือแค่ ใหญ่แค่ตัวแรก
```
...
```text
เช็คเงื่อนไขทั้งหมด เหลือติดแค่ สีซ้ำกันได้อะไร
```
