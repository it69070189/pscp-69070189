...
```text
input:
รับค่าคะแนนสูงสุดของทั้ง 3 คน และ คะแนนรวมของทั้ง 3 คน
```
...
```text
Step 1: รับค่าคะแนนสูงสุด และ คะแนนรวม
Step 2:หาคะแนนต่ำสุด
Step 3:ถ้าคะแนนของคนที่ได้คะแนนเยอะสุด ห่างจาก คนท้ายเกิน 2 ให้พิม Surprising
Step 4:ถ้าไม่เกินให้พิมพ์ Not Surprising
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก แต่เพิ่มขั้นตอนในการเช็คที่ละเอียดขึ้น ถ้าคะแนนรวมของทั้ง 3 คนน้อยกว่าหรือเท่ากับ 3 และ คะแนนมากที่สุดน้อยกว่าหรือเท่ากับ 2 และคะแนนของคนที่ได้คะแนนน้อยที่สุด น้อยกว่า 0 เพราะยังไงก็น้อยกว่าแน่ๆถ้าติดลบจึงจะไม่พิม Surprising
```
...
```text
คะแนนรวมได้ 0 แต่ได้คะแนนของคนที่สูงที่สุดได้ 1 ซึงไม่ควรจะหาคาดเดาได้
```
...
```text
คะแนนรวมได้ 0 แต่คะแนนเยอะสุด > 2 พอเอาไปคำนวณแล้วมันควรจะได้ค่าติดลบแต่พอเอาไปเข้าเงื่อนไข ดันกลายเป็นค่าบวก
```
...
```text
คะแนนรวม ได้มากกว่า 0 แต่คะแนนคนที่สูงที่สุดได้ 0 เท่ากับว่าไม่ควรจะหาได้ นอกเหนือจากนนั้นคะแนนจะติดลบแล้ว
```
...
```text
ช่วยคิด testcase ที่ซ่อนอยู่
```

คุณยังทำอะไรด้วยตนเอง

```text
คำนวณหาค่าน้อยที่สุด และทำเงื่อนไข
```
