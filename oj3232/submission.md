...
```text
โจทย์ต้องการให้หาว่ากบต้องกระโดดกี่ครั้งจนกว่าจะถึงเป้าหมาย
Input:
จุดเริ่มต้น , จุดปลายทาง
Output:
จำนวนครั้งที่กบกระโดด หากไม่ถึงแสดงผล -1
Constraints:
input อยู่ในช่วงที่จำกัด
1 <= จุดเริ่มต้น <= 1000
1 <= จุดปลายทาง <= 100000
```
...
```text
Step1: รับค่าจุดเริ่มต้นและจุดปลายทาง
Step2: คำนวณครั้งที่กบกระโดด และระยะทาง โดยให้โดดไปเรื่อยๆ จนกว่า ระยะทางที่กบกระโดดได้นั้นจะ <= 0 แล้วให้หยุด
```
...
```text
เหมือนกับแผนแรก แต่เพิ่มเงื่อนไข คือ ถ้าจุดเริ่มต้น = จุดปลายทางจะให้ ค่า 1 มาเลย และ หากกระโดดไม่ถึงให้แสดง -1
```
...
```text
เช็คว่ากระโดดถึงรึยังโดยที่หากถึงแล้วให้หยุดโดยทันทีเพราะต้องการหาจำนวนครั้งที่กบกระโดดถึงจุดปลายทางน้อยที่สุด และให้สอดคล้องกับ x-2, x-4 x-6,..x-n
...
```text
จุดเริ่มต้น มากกว่าจุดปลายทางอยู่แล้วแต่กระโดดเกิน 
```
